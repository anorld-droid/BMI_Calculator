...
class BMIRepositoryImpl @Inject constructor() : BMIRepository {
    override fun calculateBMIAndPonderalIndex(personalBMIDetails: Details): Results {
        val bmi: Double =
            personalBMIDetails.weight / ((personalBMIDetails.height * personalBMIDetails.height) / 10000)
        val ponderal: Double =
            personalBMIDetails.weight / ((personalBMIDetails.height * personalBMIDetails.height * personalBMIDetails.height) / 1000000)
        val category: String
    ...
    override fun shareScreenShot(activity: Activity, view: View) {
        ...
        try {
            ...
            view.draw(canvas)
            val imageFile =
                File(activity.getExternalFilesDir(Environment.DIRECTORY_PICTURES), "$name.jpg")
